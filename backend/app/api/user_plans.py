...
@router.get(
    "/", 
    response_model=List[uuid.UUID], # Return a list of race UUIDs
    summary="Get IDs of races in the authenticated user's plan"
)
async def get_user_plan(
    *, # Keyword-only args below
    ...
    """Retrieves a list of race IDs currently in the authenticated user's plan."""
    ...
    try:
        # Select only the race_id column
        ...
        # Extract just the race_id values from the list of dictionaries
        planned_race_ids = [item['race_id'] for item in response.data]
        return planned_race_ids

    except Exception as e:
        print(f"Error fetching plan for user {user_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail="An error occurred while fetching the user plan."
        )
